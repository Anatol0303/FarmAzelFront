        ...
        const GetMyOrderedSurprizeBacks = async () => {
            try {
                ...
                if (!response.ok) {
                    ...
                    throw new Error(data.message || 'Failed to find SurprizeBacks you ordered');
                }

                setinfMyOrderedSurprizeBacks(data);
                setSuccess(`Information of SurprizeBacks you ordered is loaded`);
            } catch (err) {
    ...
    if (roleFromToken !== 'client') {
        ...
        return (
            <Container fluid className="py-4">
                <h2 className="text-center mb-4">Your ordered SurprizeBacks</h2>

                <Form className="mb-3 text-center">

                    <Button
                        className="mt-2"
                        onClick={GetMyOrderedSurprizeBacks}
                        disabled={!loginFromToken || !token}
                    >
                        Load
                    </Button>
                </Form>
                ...
                <div className="row justify-content-center">
                    {infMyOrderedSurprizeBacks?.length && infMyOrderedSurprizeBacks.map((sb, index) => (
...
export default InfMyOrderedSurprizeBacks;